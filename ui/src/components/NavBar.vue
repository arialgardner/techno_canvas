...
export default {
  ...
  setup(props) {
    ...
    // Using RTDB for consistent presence tracking across all users
    const { activeUsers, activeUsersVersion, getActiveUsers, getActiveUserCount, setUserOffline } = usePresenceRTDB()
    
    const { removeCursor } = useCursorsRTDB()
